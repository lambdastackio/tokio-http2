...
//!
//! tokio-http2 has the opinion that Headers should be strongly-typed, because that's
...
//!
//! tokio-http2 provides many of the most commonly used headers in HTTP. If
//! you need to define a custom header, it's easy to do while still taking
//! advantage of the type system. tokio-http2 includes a `header!` macro for defining
//! many wrapper-style headers.
//!
//! ```
//! #[macro_use] extern crate tokio-http2;
//! use tokio_http2::header::Headers;
...
//! ```
//! use std::fmt;
//! use tokio_http2::header::{Header, Raw};
//!
...
//!     }
//!
//!     fn parse_header(raw: &Raw) -> tokio_http2::Result<Dnt> {
//!         if raw.len() == 1 {
//!             let line = &raw[0];
//!             if line.len() == 1 {
//!                 let byte = line[0];
...
fn test_get_type() {
    ...
    let agent = UserAgent("tokio-http2".to_owned());
...
impl Headers {
    ...
    /// ```
    /// # use tokio_http2::header::Headers;
    /// # use tokio_http2::header::ContentType;
    /// # let mut headers = Headers::new();
